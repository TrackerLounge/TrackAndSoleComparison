...
/* 
 ...
 Code modified slightly to calculate imgObject descriptor fewer times.
 We could have modified the code to ensure all descriptors are calculated once and stored for reuse 
 but I didn't want to optimize the workflow until I had knew if it would produce usable results.
 
 Code also modified to protect against some opencv thrown exception cases
*/
public class SurfFeatuerHomography {
	String filename1;
	Mat imgObject;
	...
	public SurfFeatuerHomography(String filename1) {
		this.filename1 = filename1;
		this.imgObject = Imgcodecs.imread(filename1, Imgcodecs.IMREAD_GRAYSCALE);
		if (this.imgObject.empty()) {
			System.err.println("Cannot read images!");
			System.exit(0);
		}
		this.keypointsObject = new MatOfKeyPoint();
		this.descriptorsObject = new Mat();
		this.detector.detectAndCompute(imgObject, new Mat(), keypointsObject, descriptorsObject);
	}

	public int run(String filename2, boolean showResult) {

		Mat imgScene = Imgcodecs.imread(filename2, Imgcodecs.IMREAD_GRAYSCALE);
		if (this.imgObject.empty() || imgScene.empty()) {
			System.err.println("Cannot read images!");
			System.exit(0);
		}
		...
		// System.out.println("We found "+listOfGoodMatches.size()+" matches");
...
//			if (listOfGoodMatches.size() > 20) {
			...
			// System.out.println("Not enough matches to create bounding box");
		}
		int numOfMatches = 0;
		if (null != listOfGoodMatches && listOfGoodMatches.size() >= 0) {
			numOfMatches = listOfGoodMatches.size();
		}
		return numOfMatches;
	}
}
